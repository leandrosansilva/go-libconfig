...
func TestParseWithoutSemicolons(t *testing.T) {
	var p Parser
	v, err := p.Parse(`
s1 = 34;
s2 = 42
...
s6 = 4.2;
	...
	s1 := v.GetInt("s1")
	if s1 != 34 {
		t.Fatalf("got=%v expected=34", s1)
	}

	s2 := v.GetInt("s2")
	if s2 != 42 {
		t.Fatalf("got=%v expected=42", s2)
	}

	s3 := v.GetStringBytes("s3")
	if string(s3) != "test123" {
		t.Fatalf("got=%v expected=test123", s3)
	}

	s4i2 := v.GetInt("s4", "2")
	if s4i2 != 3 {
		t.Fatalf("got=%v expected=3", s4i2)
	}

	s5i1 := v.GetInt("s5", "1", "v")
	if s5i1 != 43 {
		t.Fatalf("got=%v expected=43", s5i1)
	}

	s6 := v.GetFloat64("s6")
	if s6 != 4.2 {
		t.Fatalf("got=%v expected=4.2", s6)
	}
}
